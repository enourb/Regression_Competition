...
loan_recipe <- recipe(money_made_inv ~
                        # numeric
                        out_prncp_inv + loan_amnt + bc_util +
                        # nominal
                        initial_list_status + grade + term, data = loan_train) %>%
  step_other(grade, threshold = .05) %>%
...
save(loan_fold, loan_recipe, loan_split, loan_train, file = "data/temp_05/loan_setup.rda")
...
write.csv(final_predict, file = "data/temp_05/test_pred.csv", row.names = FALSE)
